...
description: オプティカルフローセンサ PMW3901 のレジスタアクセス手順、SPI モード・クロック、製品 ID による疎通確認、移動量の読み出しと性能設定レジスタ列。
tags: [stampfly, sensor, spi]
status: draft
generated: { by: claude-opus-5/1m, at: 2026-09-06T03:25:00Z }
verified:
...
# 性能設定レジスタ列（マジックレジスタ）が必須

**リセット直後の状態では移動量をまともに検出できない。**
メーカー指定の約 70 個のレジスタ書き込みを行って初めて実用的な性能になる。

- `0x7F` はレジスタページの切り替え。以降の書き込み先ページを選ぶ
- 途中に **100ms の待ち**が要る（その後さらに 14 個書く）
- **データシートに記載の無い非公開レジスタ**で、個々の値の意味は公開されていない。
  PixArt / Bitcraze が実測で決めた値なので、順序と値をそのまま守るしかない

出典: [Bitcraze_PMW3901](https://github.com/bitcraze/Bitcraze_PMW3901)
(MIT License, Copyright (c) 2017 Bitcraze AB)。本リポジトリも MIT なので
表記を残せば同梱できる。
...
電源投入から初期化完了までの間の動きが、最初の 1 回の読み出しに乗ってくる。
`Flow_init()` の最後で 1 回読んで捨てる。
